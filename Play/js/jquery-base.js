...
$(function(){
	...
	$(window).scroll(function(){
		...
		if(scrollTop>=618){
			$('.max_width').css({'position':'fixed','left':0,'top':0,'z-index':9999999});
			
			if($('#menuBg').length==0) $('.max_width').after('<div id="menuBg" style="height:53px; overflow:hidden;"></div>')
		}else{
			if($('#menuBg').length>0) $('#menuBg').remove();
			
			$('.max_width').removeAttr('style');
		}
	});
});