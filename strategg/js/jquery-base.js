...
$(function(){
	...
	$(window).load(function(){
		getMenuIndex=$('.header-bot .menu li.on').index();
	});
	$('.header-bot .menu li').hover(function(){
		$(this).addClass('on').siblings().removeClass('on');
		$(this).find('ul').show();
	},function(){
		$('.header-bot .menu li').eq(getMenuIndex).addClass('on').siblings().removeClass('on')
		$(this).find('ul').hide();
	});
	//滚动到顶部
	...
	$(window).scroll(function(){
		...
		if(scrollTop>=618){
			$('.subnav').css({'position':'fixed','left':0,'top':0,'z-index':9999999});
			
			if($('#subnav').length==0) $('.subnav').after('<div id="menuBg" style="height:53px; "></div>')
		}else{
//			if($('#subnav').length>0) $('#subnav').remove();
